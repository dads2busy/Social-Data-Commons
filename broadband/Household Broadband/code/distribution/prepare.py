...
Splits the ingest output into per-level dashboard files for VA and NCR.
"""
...
def find_source(dist_dir: Path, prefix: str) -> Path | None:
    ...
    candidates = sorted(dist_dir.glob(f"{prefix}_*census_acs*household_broadband*.csv.xz"))
...
def run() -> None:
    ...
    if va_source:
        ...
        log.warning("No VA household broadband source file found in %s", DIST_DIR)
    ...
    if ncr_source:
        ...
        log.warning("No NCR household broadband source file found in %s", DIST_DIR)
