"""Prepare Industry employment metrics for dashboard sites."""
...
log = get_logger("employment.industry.prepare")
...
def run() -> None:
    ...
    title = "employment_metrics_by_Industry"
    ...
    for prefix in config["sources"]:
        ...
        # HHI files (county only)
        hhi_source = find_source(DIST_DIR, prefix, "herfindalh_index_by_industry")
        if hhi_source and dashboard_rel:
            paths = data_reformat_for_site(
                source_path=hhi_source, output_dir=REPO_DIR / dashboard_rel,
                levels=["county"], coverage_area=prefix, data_source="mi",
        ...
        # LQ files
        lq_source = find_source(DIST_DIR, prefix, "location_quotient_by_industry")
        if lq_source and dashboard_rel:
            paths = data_reformat_for_site(
                source_path=lq_source, output_dir=REPO_DIR / dashboard_rel,
                levels=["block_group", "tract", "county"], coverage_area=prefix,
                data_source="mi", title="location_quotient_by_industry",
            )
            for p in paths:
                log.info("Wrote %s", p)
