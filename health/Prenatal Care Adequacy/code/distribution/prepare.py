...
Steps:
1. Read VA and NCR ingest output
...
def find_va_source(dist_dir: Path) -> Path | None:
    """Find VA ingest output, excluding prepare (hdct) output."""
    candidates = sorted(
        p for p in dist_dir.glob("va_ct_nchs_*_kotelchuck.csv.xz")
...
def find_ncr_source(dist_dir: Path) -> Path | None:
    ...
    candidates = sorted(dist_dir.glob("ncr_ct_nchs_*_kotelchuck.csv.xz"))
    return candidates[-1] if candidates else None
