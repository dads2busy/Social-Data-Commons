"""Prepare mental and physical healthy days for dashboard sites.

Steps:
1. Read VA and NCR ingest output (tract + county)
2. Aggregate VA county data to health districts via crosswalk
3. Write combined VA distribution file and reformat for VA dashboard
4. Reformat NCR distribution file for NCR dashboard
"""
...
log = get_logger("healthy_days.prepare")
...
def find_va_source(dist_dir: Path) -> Path | None:
    """Find VA ingest output, excluding prepare (hdcttr) output."""
    candidates = sorted(
        p for p in dist_dir.glob("va_cttr_cdc_places_*_healthy_days.csv.xz")
...
def find_ncr_source(dist_dir: Path) -> Path | None:
    """Find NCR ingest output."""
    candidates = sorted(dist_dir.glob("ncr_cttr_cdc_places_*_healthy_days.csv.xz"))
    return candidates[-1] if candidates else None


def build_va_with_health_districts(va_source: Path, crosswalk_path: Path) -> Path:
    """Aggregate VA county data to health districts and write combined file."""
