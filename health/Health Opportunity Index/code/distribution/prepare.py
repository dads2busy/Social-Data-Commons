...
def find_source(dist_dir: Path) -> Path | None:
    candidates = sorted(
        p for p in dist_dir.glob("va_*health_opportunity_index*.csv.xz")
        if "hdcttr" not in p.name
    )
    return candidates[-1] if candidates else None


def fetch_tract_population(client: CensusClient, data_years: list[int]) -> pd.DataFrame:
    """Fetch VA tract-level total population for weighting.

    Returns DataFrame with columns: geoid, year, pop
    Each data year uses ACS from the same year (or nearest available).
    """
    pop_frames: list[pd.DataFrame] = []
    cached_pop: dict[int, pd.DataFrame] = {}

    for data_year in data_years:
        # ACS 5-year available 2010-2024; use same year
        acs_year = min(data_year, 2024)
        if acs_year not in cached_pop:
            log.info("Fetching ACS %d tract population", acs_year)
            ...
            cached_pop[acs_year] = pop[["geoid", "pop"]].copy()

        frame = cached_pop[acs_year].copy()
        frame["year"] = data_year
        pop_frames.append(frame)
...
def run() -> None:
    ...
    client = CensusClient()
    data_years = sorted(tracts["year"].unique().tolist())
    pop_df = fetch_tract_population(client, data_years)
    ...
    if va_source != out_path and va_source.exists():
        va_source.unlink()
