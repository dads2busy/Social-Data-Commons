"""Prepare urgent care access scores for dashboard sites.
...
def find_va_source(dist_dir: Path) -> Path | None:
    ...
    candidates = sorted(
        p for p in dist_dir.glob(f"va_cttrbg_{DATA_SOURCE}*access_scores_{MEASURE_PREFIX}*.csv.xz")
...
def build_va_with_health_districts(va_source: Path, crosswalk_path: Path) -> Path:
    ...
    # Count measures need sum; FCA and travel time measures need mean.
