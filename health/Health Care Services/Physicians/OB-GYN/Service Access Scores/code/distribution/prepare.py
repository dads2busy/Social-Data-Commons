"""Prepare OB-GYN access scores for dashboard sites.
...
def find_va_source(dist_dir: Path) -> Path | None:
    """Find VA ingest output (county+tract+BG, no HD, CMS source)."""
    candidates = sorted(
        p for p in dist_dir.glob(f"va_cttrbg_{DATA_SOURCE}_*access_scores_{MEASURE_PREFIX}*.csv.xz")
...
def find_ncr_source(dist_dir: Path) -> Path | None:
    """Find NCR ingest output (county+tract+BG, CMS source)."""
    candidates = sorted(
        dist_dir.glob(f"ncr_cttrbg_{DATA_SOURCE}_*access_scores_{MEASURE_PREFIX}*.csv.xz")
    )
    return candidates[-1] if candidates else None
