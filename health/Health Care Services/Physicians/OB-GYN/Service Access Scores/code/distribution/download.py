"""Download CMS Doctors and Clinicians data and geocode OB-GYN provider addresses.

Downloads CMS archive ZIPs for 2017-2025, filters to VA/DC/MD OB-GYN
physicians, and saves per-year CSVs.  Then geocodes new addresses via the
Census Geocoder API, caching results from prior runs.
"""
...
# CMS column name variants across years (2017 has verbose names, 2018+ abbreviated)
CMS_COLUMN_MAP = {
    # NPI
    'NPI': 'npi',
    'npi': 'npi',
    ...
    'Pri_Spec': 'primary_specialty',
    'Secondary specialty 1': 'secondary_specialty_1',
    'sec_spec_1': 'secondary_specialty_1',
    'Sec_Spec_1': 'secondary_specialty_1',
    'Secondary specialty 2': 'secondary_specialty_2',
    'sec_spec_2': 'secondary_specialty_2',
    'Sec_Spec_2': 'secondary_specialty_2',
    # Address
    'Line 1 Street Address': 'address_line_1',
    'adr_ln_1': 'address_line_1',
    'Adr_Ln_1': 'address_line_1',
    'Line 2 Street Address': 'address_line_2',
...
def download_and_filter_year(year: int) -> pd.DataFrame:
    """Download CMS ZIP for a year, filter to VA/DC/MD OB-GYN, return DataFrame."""
    url = cms_url(year)
    ...
    with zipfile.ZipFile(io.BytesIO(resp.content)) as zf:
        ...
        with zf.open(csv_name) as f:
            dt = pd.read_csv(f, dtype=str, low_memory=False)
    ...
    # Filter: state in VA/DC/MD, credential MD/DO, OB-GYN specialty
    ...
    spec_mask = (
        dt['primary_specialty'].isin(OBGYN_SPECIALTIES)
        | dt['secondary_specialty_1'].isin(OBGYN_SPECIALTIES)
        | dt['secondary_specialty_2'].isin(OBGYN_SPECIALTIES)
    )
...
def download_all(years: list[int] | None = None, skip_existing: bool = True) -> None:
    ...
    for year in years:
        out_path = ORIGINAL_DIR / f'vadcmd_cms_{year}_obgyn.csv'
...
def standardize_existing_files(years: list[int] | None = None) -> None:
    ...
    for year in years:
        path = ORIGINAL_DIR / f'vadcmd_cms_{year}_obgyn.csv'
        if not path.exists():
            continue
        df = pd.read_csv(path, dtype=str, low_memory=False)
        needs_standardize = any(col in df.columns for col in [
            'NPI', 'lst_nm', 'Last Name', 'Primary specialty', 'Line 1 Street Address',
        ])
        if needs_standardize:
            log.info('Standardizing columns for %s', path.name)
            df = _standardize_columns(df)
            df['year'] = year
            # Keep only the standard output columns
...
def build_combined_working_file(years: list[int] | None = None) -> Path:
    ...
    out_path = WORKING_DIR / 'vadcmd_cms_2017_2025_obgyn_physicians.csv'
...
def geocode_new_addresses(years: list[int] | None = None) -> Path:
    ...
    existing geocoded cache, geocodes new ones, and writes an updated cache.
    """
    if years is None:
        years = ALL_YEARS
    ...
    WORKING_DIR.mkdir(parents=True, exist_ok=True)
    geo_cache_path = WORKING_DIR / 'vadcmd_cms_2017_2025_obgyn_physicians_geo.csv'

    existing_geo = pd.DataFrame()
    if geo_cache_path.exists():
        existing_geo = pd.read_csv(geo_cache_path, dtype={'postalcode': str})
        log.info('Loaded geocode cache from %s (%d rows)', geo_cache_path.name, len(existing_geo))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Download CMS OB-GYN data and geocode')
    ...
    # Standardize old-format per-year CSVs (2017-2022 from R code)
