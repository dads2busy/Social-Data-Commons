...
Uses CMS Doctors and Clinicians data (PEDIATRIC MEDICINE specialty), ACS
population ages 0-17 at block group level, and pre-computed BG-to-BG travel
times to compute 2SFCA, E2SFCA, and 3SFCA access scores.  Loops over all
available years (2018-2025) and writes combined multi-year BG+tract+county
...
def load_geocoded_addresses(config: dict) -> pd.DataFrame:
    ...
    if not geo_path.exists():
        # Fall back to Primary Care geocoded file (same CMS addresses)
        primcare_path = (
            TOPIC_DIR.parents[1] / "Primary Care" / "Service Access Scores"
            / "data" / "working" / "vadcmd_cms_2018_2025_primary_care_physicians_geo.csv"
        )
        if primcare_path.exists():
            log.warning("Geocoded file not found at %s, falling back to %s", geo_path.name, primcare_path.name)
            geo_path = primcare_path
        else:
            raise FileNotFoundError(f"No geocoded file found at {geo_path} or {primcare_path}")
...
def load_providers_for_year(
    ...
    """
    cms_path = ORIGINAL_DIR / f"vadcmd_cms_{year}_pediatric.csv"
    ...
    # Filter to pediatric medicine specialties (check all 3 spec columns)
    spec_mask = (
    ...
    log.info("CMS %d pediatric rows: %d (%d unique NPIs)", year, len(pc), pc["npi"].nunique())
