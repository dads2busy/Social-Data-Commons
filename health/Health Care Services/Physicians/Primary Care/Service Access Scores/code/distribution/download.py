...
Downloads CMS archive ZIPs for 2018-2025, filters to VA/DC/MD primary care
...
log = get_logger('primcare.download')
...
def download_and_filter_year(year: int) -> pd.DataFrame:
    """Download CMS ZIP for a year, filter to VA/DC/MD primary care, return DataFrame."""
    url = cms_url(year)
    ...
    # Filter: state in VA/DC/MD, credential MD/DO, primary care specialty
    ...
    spec_mask = (
        dt['primary_specialty'].isin(PRIMARY_CARE_SPECIALTIES)
        | dt['secondary_specialty_1'].isin(PRIMARY_CARE_SPECIALTIES)
        | dt['secondary_specialty_2'].isin(PRIMARY_CARE_SPECIALTIES)
    )
...
def download_all(years: list[int] | None = None, skip_existing: bool = True) -> None:
    ...
    for year in years:
        out_path = ORIGINAL_DIR / f'vadcmd_cms_{year}_primary.csv'
...
def standardize_existing_files(years: list[int] | None = None) -> None:
    ...
    for year in years:
        path = ORIGINAL_DIR / f'vadcmd_cms_{year}_primary.csv'
        if not path.exists():
            continue
        ...
        if 'NPI' in df.columns or 'lst_nm' in df.columns:
            ...
            df['year'] = year
            # Keep only the standard output columns (some old files may lack year)
...
def build_combined_working_file(years: list[int] | None = None) -> Path:
    ...
    for year in years:
        ...
        if path.exists():
            ...
            # Handle old-format files that may still have CMS column names
            df = _standardize_columns(df)
    ...
    out_path = WORKING_DIR / 'vadcmd_cms_2018_2025_primary_care_physicians.csv'
...
def geocode_new_addresses(years: list[int] | None = None) -> Path:
    ...
    WORKING_DIR.mkdir(parents=True, exist_ok=True)
    geo_cache_path = WORKING_DIR / 'vadcmd_cms_2018_2025_primary_care_physicians_geo.csv'
    old_cache_path = WORKING_DIR / 'vadcmd_cms_2018_2022_primary_care_physicians_geo_tmp.csv'

    existing_geo = pd.DataFrame()
    for p in [geo_cache_path, old_cache_path]:
        if p.exists():
    ...
    if not existing_geo.empty and all(c in existing_geo.columns for c in geo_key_cols):
        ...
        # Keep unique geocoded rows (deduplicate by address)
    ...
    if not existing_geo_dedup.empty:
        # Ensure column alignment — existing cache has npi column, new does not
        # For the geo cache we store address-level data (no npi)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Download CMS primary care data and geocode')
    ...
    # Standardize old-format per-year CSVs (2018-2022 from R code)
