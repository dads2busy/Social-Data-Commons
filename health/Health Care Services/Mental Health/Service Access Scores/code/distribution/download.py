"""Download mental health treatment facilities from SAMHSA findtreatment.gov API.

Paginates through the SAMHSA treatment locator API (sType=MH), filters to
...
log = get_logger('mental.download')
...
def download_all_facilities() -> pd.DataFrame:
    """Download all MH facilities from SAMHSA, paginating through all pages.

    Saves progress to a temp JSON file so partial downloads can resume.
    """
    WORKING_DIR.mkdir(parents=True, exist_ok=True)
    progress_path = WORKING_DIR / '_mh_download_progress.json'
    ...
    if start_page == 1:
        ...
        log.info('SAMHSA MH: %d total records, %d pages', record_count, total_pages)
        all_rows = list(result.get('rows', []))
        start_page = 2
    else:
        # Re-fetch page 1 just to get total_pages
        result = fetch_page(1)
        total_pages = result['totalPages']

    for page in range(start_page, total_pages + 1):
        time.sleep(1.5)
        # Extra cooldown every 50 pages to avoid rate limit
        if page % 50 == 0:
...
def run() -> Path:
    """Download SAMHSA MH facilities and write to working directory."""
    WORKING_DIR.mkdir(parents=True, exist_ok=True)
    out_path = WORKING_DIR / 'samhsa_2025_mental_health_facilities.csv'
