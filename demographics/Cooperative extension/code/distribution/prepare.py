"""Prepare cooperative extension measures for dashboard sites.

Reads ingest output, aggregates county rows to health districts for VA,
and reformats for VA and NCR dashboards.
"""
...
def find_source(dist_dir: Path, prefix: str) -> Path | None:
    """Find ingest output but not prepare output.

    Ingest writes {prefix}_cttr_mixed* (county+tract); prepare writes
    {prefix}_hdcttr_mixed* (adds HD). Match only cttr to avoid picking
    up prepare output.
    """
    candidates = sorted(
        dist_dir.glob(f"{prefix}_cttr_mixed*cooperative_extension*.csv.xz")
    )
...
def run() -> None:
    ...
    # VA source: aggregate to health districts + reformat for VA dashboard
    ...
    # NCR source: reformat directly for NCR dashboard (no HD aggregation)
    ...
    if ncr_source:
        ...
        log.warning("No NCR ingest output found in %s", DIST_DIR)
