...
def find_source(dist_dir: Path, prefix: str) -> Path | None:
    ...
    candidates = sorted(dist_dir.glob(f"{prefix}_*cnt*affordability_index*.csv.xz"))
...
def run() -> None:
    ...
    if va_source:
        ...
        log.warning("No VA affordability source file found in %s", DIST_DIR)
    ...
    if ncr_source:
        ...
        log.warning("No NCR affordability source file found in %s", DIST_DIR)
