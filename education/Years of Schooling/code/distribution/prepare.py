"""Prepare average years of schooling for dashboard sites.

Steps:
1. Find VA and NCR distribution files produced by ingest.py
2. For VA: aggregate county -> health district, write combined VA file,
   reformat for VA dashboard (health_district, county, tract)
...
def find_source(dist_dir: Path, prefix: str) -> Path | None:
    # Match ingest output (county+tract only, "cttr") not prepare output (includes "hd")
    candidates = sorted(dist_dir.glob(f"{prefix}_cttr_census_acs*years_of_schooling*.csv.xz"))
