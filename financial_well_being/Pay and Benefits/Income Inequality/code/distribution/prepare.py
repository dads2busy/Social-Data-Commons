"""Prepare income inequality (Gini index) for dashboard sites."""
...
log = get_logger("income_inequality.prepare")
...
def find_source(dist_dir: Path, prefix: str) -> Path | None:
    # Match ingest output (cttr) — not prepare output (hdcttr)
    candidates = sorted(dist_dir.glob(f"{prefix}_cttr_census_acs*income_inequality*.csv.xz"))
...
def build_va_with_health_districts(va_source: Path, crosswalk_path: Path) -> Path:
    ...
    # Gini index is a continuous measure — aggregate to HD using mean
