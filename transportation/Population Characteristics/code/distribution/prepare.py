"""Prepare transportation population characteristics for dashboard sites.
...
log = get_logger("population_characteristics.prepare")
...
def find_source(dist_dir: Path, prefix: str) -> Path | None:
    candidates = sorted(dist_dir.glob(f"{prefix}_cttr_census_acs*population_characteristics*.csv.xz"))
...
def run() -> None:
    ...
    if ncr_source:
        ...
        for p in data_reformat_for_site(
            ...
            levels=["county", "tract"],
            coverage_area="ncr", data_source="census_acs",
            title="population_characteristics",
            measure_info_path=measure_info,
        ):
            log.info("Wrote %s", p)
