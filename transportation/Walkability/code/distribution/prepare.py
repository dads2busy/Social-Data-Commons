...
def find_source(dist_dir: Path, prefix: str) -> Path | None:
    ...
    ingest = sorted(dist_dir.glob(f"{prefix}_cttr_bi*walkability*.csv.xz"))
    if ingest:
        return ingest[-1]
    prepare = sorted(dist_dir.glob(f"{prefix}_hdcttr_bi*walkability*.csv.xz"))
...
def run() -> None:
    ...
    if va_source:
        ...
        for p in data_reformat_for_site(
            ...
            coverage_area="va", data_source="bi",
    ...
    if ncr_source:
        ...
        for p in data_reformat_for_site(
            ...
            levels=["county", "tract"],
            coverage_area="ncr", data_source="bi",
            title="walkability_index",
            measure_info_path=measure_info,
        ):
            log.info("Wrote %s", p)
