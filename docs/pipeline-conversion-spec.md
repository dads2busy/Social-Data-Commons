...
## 5.1 `measure_info.json` field standard

Every variable in `measure_info.json` must have these three fields generated by the converting developer (Claude):

### `long_description`

A 2–4 sentence narrative explaining:
1. What the measure captures
2. Why it matters for the relevant domain (NOT always public health — broadband is about digital equity, transportation about mobility, etc.)

Example: "The incarceration rate estimates how many people per 100,000 residents are held in jail, allocated to the census tracts where they lived before incarceration. Communities with high rates experience disproportionate removal of residents into the criminal justice system, which disrupts families, reduces household income, and weakens social networks."

Do NOT put ACS table references or technical methodology in `long_description` — that belongs in `provenance`.

### `short_description`

One sentence summarizing the measure. Should be distinct from `long_description`, not just the first sentence.

Example: "Jail incarceration rate per 100,000 residents, allocated to tracts by pre-incarceration residence."

### `provenance`

Technical record of how the data was produced: source datasets, transformations, aggregation methods, and any methodological choices.

Example: "Tract-level incarceration counts from Prison Policy Initiative (PPI) 2020, which estimates pre-incarceration residence using Bureau of Justice Statistics survey data applied to 2020 Census group quarters. County-level annual jail population from Vera Institute (mean of quarterly observations, 2016-2023)."
