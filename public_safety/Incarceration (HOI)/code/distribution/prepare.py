"""Prepare incarceration data for VA and NCR dashboards.
...
log = get_logger("incarceration.prepare")
...
def find_source(dist_dir: Path) -> Path | None:
    ...
    candidates = list(dist_dir.glob("va_tr_vera_ppi*incarceration*.csv.xz"))
    if candidates:
        return max(candidates, key=lambda p: p.stat().st_mtime)
    # Fall back to combined file from previous prepare run
    candidates = list(dist_dir.glob("va_*vera_ppi*incarceration*.csv.xz"))
    if candidates:
        return max(candidates, key=lambda p: p.stat().st_mtime)
    return None
...
def run() -> RunResult:
    ...
    try:
        ...
        if not source:
            return RunResult(
                success=False,
                error="No ingest output found in data/distribution/",
        ...
        return RunResult(
            success=True,
            rows=len(va_combined),
            output_path=str(va_path),
            duration_sec=time.time() - t0,
        )
